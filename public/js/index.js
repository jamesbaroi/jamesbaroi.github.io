...
const createPostElements = () => {
  ...
  src.setAttribute('class', 'c3 flex p right-align')
...
const populatePostElements = (d, o, i) => {
  ...
  !d[i].dat ? o.dat.innerHTML = '' : o.dat.innerHTML =`
    ...
    <span class="c4">Posted on:</span>
    <time datetime="${d[i].dat}" class="c4">${d[i].dat}</time>
  `
...
const implementIndex = () => {
  ...
  if (developingActive) {
    ...
    fetch('/json/index.json').then(o => o.json()).then(d => renderPosts(d))
